...
func TestControllerRotationCenter(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			func(a Align) controller {
				ctr := newController(a)
				return *ctr.Move(2, 4).Scale(5, 3).Rotate(30)
			},
			alignHelper[Vector]{
				Center:         Vector{2, 4},
				TopCenter:      Vector{2, 4},
		...
		t.Run(tc.desc, func(t *testing.T) {
			tc.expected.Execute(func(a Align, expected Vector) {
				...
				if x != expected.X || y != expected.Y {
					t.Fatalf("[%s] expected %v, but got %.2f, %.2f", a.String(), expected, x, y)
				}
			})
		})
	}
...
func TestControllerVertexes(t *testing.T) {
	...
	testCases := []struct {
		...
		// 	func(a Align) controller {
		// 		ctr := newController(a)
		// 		return *ctr.Move(8, 6).Scale(3, 2).Rotate(90)
		// 	},
		// 	alignHelper[[]Vector]{
	...
	for _, tc := range testCases {
		t.Run(tc.desc, func(t *testing.T) {
			tc.expected.Execute(func(a Align, expected []Vector) {
				vs := getVertexes(float64(size), float64(size), tc.ctr(a))
